...
$scope.lastEditOpenedNode = null;    
    ...
    $scope.processTree = function(parentId) {
        ...
        //}
        var d = $scope.categoryList.filter(function (cat) { return cat.parentCategory == parentId });
        
        for(var c = 0; c < d.length; c++) {
            console.log(d[c]);    
            if(parentId == 0) {
                console.log('adding root '+d[c].categoryName)
                $scope.itemList.push({_id: d[c]._id, categorySlug: d[c].categorySlug, name: d[c].categoryName, parentCategory: 0, editCat: false, expandNode: true, isValid: true, nodes: []});   
            }else {
                ...
                if(parent == null) {
                    ...
                    parent.nodes.push({_id: d[c]._id, categorySlug: d[c].categorySlug, name: d[c].categoryName,  parentCategory: d[c].parentCategory, expandNode: true, editCat: false, isValid: true, nodes: []}); 
    ...
    $scope.addNode = function(categoryName, parentCategory) {
        ...
        if(!(parentCategory === undefined) && parentCategory != null && categoryName != parentCategory && parentCategory != 0) {
            ...
            if(parent == null)
                $scope.itemList.push({name: categoryName,nodes: []});    
            else {
                //console.log(parent);
                parent.nodes.push({name: categoryName,nodes: []});    
            }
        }else {
            $scope.itemList.push({name: categoryName,nodes: []});
        }
       ...
       for (var y = 0; y < steps.length; y++) {
          if (y==0) {
             if (!$scope.tree.children||typeof $scope.tree.children == 'undefined'){
                $scope.tree = { text: steps[y], leaf: false, children: [] };
             }
